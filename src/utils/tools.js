 ...
 */
 define(function(){
            ...
            Shuffling : function (container, direction, els, delay, next, prev) {
                ...
                //实现吸顶效果
                Sucktop:function (header, content, win, ment, cssstyle, topcontent) {   // (jq版)
                ...
                //弹框
                Bounced:function (Popup, body, event, x) { //参数分别为弹框体、遮罩、事件源（点击出现弹框）、关闭弹框按钮 (jq版)
                    //弹框实现弹出
                ...
                //手机号正则表达式判断
                Mobile:function (phone, prompt) {  //phone:input输入手机号的value值进行判断;prompt:要提示的语句
                ...
                //随机验证码判断
                code:function (code1, code2, prompt) {
                    if (code1 == code2) {
                        prompt.css({
                            opacity: 0,
                        })
                ...
                //再次验证密码
                psda:function (psdA, psdB, prompt) {
                    if (psdA === psdB) {
                        prompt.css({
                            opacity: 0,
                        })
